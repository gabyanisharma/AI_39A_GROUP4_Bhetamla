...
class MeetingAnalytics:
    ...
    @staticmethod
    def get_attendee_names(user_id):
        """meetup_id -> comma-joined attendee names, for this user's meetups (F24 search)."""
        query = """
            SELECT mm.meetup_id,
                   GROUP_CONCAT(u.full_name ORDER BY u.full_name SEPARATOR ', ') AS attendees
            FROM meetup_members mm
            JOIN users u ON u.id = mm.user_id
            WHERE mm.meetup_id IN (
                SELECT meetup_id FROM meetup_members WHERE user_id = %s
            )
            GROUP BY mm.meetup_id
        """
        rows = execute_query(query, (user_id,), fetch=True) or []
        return {r['meetup_id']: r['attendees'] for r in rows}

    @staticmethod
    def get_notes(user_id):
        """meetup_id -> note text for the user (F24 add notes)."""
        rows = execute_query(
            "SELECT meetup_id, note FROM meeting_notes WHERE user_id = %s",
            (user_id,), fetch=True
        ) or []
        return {r['meetup_id']: r['note'] for r in rows}

    @staticmethod
    def save_note(user_id, meetup_id, note):
        execute_query(
            """
            INSERT INTO meeting_notes (user_id, meetup_id, note)
            VALUES (%s, %s, %s)
            ON DUPLICATE KEY UPDATE note = VALUES(note), updated_at = CURRENT_TIMESTAMP
            """,
            (user_id, meetup_id, note)
        )
